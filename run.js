...
const changeView = (() => {
    ...
    (() => {
        ...
        const startDragging = ev => {
            mouseDown = true;
            startX = ev.pageX - EL_MAIN.offsetLeft;
            startY = ev.pageY - EL_MAIN.offsetTop;
            scrollLeft = EL_MAIN.scrollLeft;
            scrollTop = EL_MAIN.scrollTop;
        };
        ...
        const move = ev => {
            ev.preventDefault();
            if (!mouseDown) { return; }
            const x = ev.pageX - EL_MAIN.offsetLeft;
            const y = ev.pageY - EL_MAIN.offsetTop;
            const scrollX = x - startX;
            const scrollY = y - startY;
            EL_MAIN.scrollLeft = scrollLeft - scrollX;
            EL_MAIN.scrollTop = scrollTop - scrollY;
        };

        // Add the event listeners
        EL_MAIN.addEventListener('mousemove', move, false);
        EL_MAIN.addEventListener('mousedown', startDragging, false);
        EL_MAIN.addEventListener('mouseup', stopDragging, false);
        EL_MAIN.addEventListener('mouseleave', stopDragging, false);
    })();

    // ZOOM
    const zoomPage = () => {
        // EL_TREE.style.scale = +EL_ZOOM.value / 100;
        const newScale = +EL_ZOOM.value / 100;

        // 1. Get current dimensions/scroll
        const width = EL_MAIN.clientWidth;
        const height = EL_MAIN.clientHeight;

        // 2. Calculate the point we are looking at (the center of the 'main' container)
        // We add scrollLeft to the half-width to find the point relative to the tree's start
        const centerX = EL_MAIN.scrollLeft + width / 2;
        const centerY = EL_MAIN.scrollTop + height / 2;

        // 3. Apply the new scale
        EL_TREE.style.scale = newScale;

        // 4. Calculate the ratio of change
        const ratio = newScale / currentScale;

        // 5. Update scroll positions to keep the center point stable
        EL_MAIN.scrollLeft = (centerX * ratio) - (width / 2);
        EL_MAIN.scrollTop = (centerY * ratio) - (height / 2);

        // 6. Update currentScale for the next event
        currentScale = newScale;
    };
    EL_ZOOM.oninput = zoomPage;

    // SCROLL WHEEL ZOOM
    EL_MAIN.onwheel = ev => {
        ev.preventDefault();
        EL_ZOOM.value = Math.min(
            Math.max(
                +EL_ZOOM.min,
                +EL_ZOOM.value + (ev.deltaY * -0.01),
            ),
            +EL_ZOOM.max
        );
        zoomPage();
    ...
    (() => {
        ...
        EL_MAIN.addEventListener('touchstart', ev => {
            if (ev.touches.length === 2) {
                ev.preventDefault(); // Prevent scrolling while pinching
                initialDist = getDist(ev);
                initialZoom = +EL_ZOOM.value;
            }
        }, { passive: false });

        EL_MAIN.addEventListener('touchmove', ev => {
            if (ev.touches.length === 2 && initialDist > 0) {
                ev.preventDefault();

                const currentDist = getDist(ev);

                // Calculate how much the distance changed relative to start
                // Sensitivity (0.1) can be adjusted to make zoom faster/slower
                const diff = (currentDist - initialDist) * 0.5;

                const newZoom = Math.min(
                    Math.max(+EL_ZOOM.min, initialZoom + diff),
                    +EL_ZOOM.max
                );

                EL_ZOOM.value = newZoom;
                zoomPage(); // Reuse your existing zoomPage logic
            }
        }, { passive: false });

        EL_MAIN.addEventListener('touchend', () => {
            initialDist = 0;
        });
    })();
